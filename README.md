...
사건이 조사 중일 때 사건 상세에 격리 브라우저 화면이 약 0.7초마다 갱신되고, 조사가 끝나면 전 과정 녹화(WebM)가
...
기획서 2.6절의 D1(가상 브랜드 의심 60개 + 헷갈리는 정상 30개, 템플릿 계열 단위로 개발용/평가용 분할)과
D2(공격 시나리오 15종)를 `eval/`에 두었다. 구성·분할 규칙·실행 방법은 [`eval/README.md`](eval/README.md).
평가용(eval)은 규칙·모델을 고정한 뒤 한 번만 연다(`run.py --split eval --frozen-evaluation`).
